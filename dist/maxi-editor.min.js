class MaxiEditor {
    ...
    static init(e) { e.registerCommand("insertLink", () => { InsertLinkPlugin.showLinkInputModal(e) }) } static showLinkInputModal(e) {
        ...
        `, document.body.appendChild(t); let i = window.getSelection(), n = i.rangeCount > 0 ? i.getRangeAt(0) : null, s = t.querySelector("#insert-link-btn"), r = t.querySelector("#cancel-btn"); s.addEventListener("click", () => { let e = t.querySelector("#link-url").value; if (e && n) { let s = document.createElement("a"); s.href = e, s.textContent = n.toString(), n.deleteContents(), n.insertNode(s), i.removeAllRanges(); let r = document.createRange(); r.selectNode(s), i.addRange(r) } t.remove() }), r.addEventListener("click", () => { t.remove() })
    }
} const styleTag = document.createElement("style"); styleTag.innerHTML = modalStyle, document.head.appendChild(styleTag);